# 如何使用 pyproject.toml 设置 LangGraph 应用程序 (How to Set Up a LangGraph Application with pyproject.toml)

必须使用 [LangGraph 配置文件](../reference/cli.md#configuration-file) 配置 LangGraph 应用程序，以便将其部署到 LangGraph Platform（或进行自托管）。本操作指南讨论了使用 `pyproject.toml` 定义包依赖项来设置 LangGraph 应用程序以进行部署的基本步骤。

本演练基于 [此存储库](https://github.com/langchain-ai/langgraph-example-pyproject)，您可以试用它以了解有关如何设置 LangGraph 应用程序以进行部署的更多信息。

!!! tip "使用 requirements.txt 设置"
    如果您更喜欢使用 `requirements.txt` 进行依赖项管理，请查看 [此操作指南](./setup.md)。

!!! tip "使用 Monorepo 设置"
    如果您有兴趣部署位于 monorepo 内的图，请查看 [此](https://github.com/langchain-ai/langgraph-example-monorepo) 存储库以获取有关如何执行此操作的示例。
...
示例 `agent.py` 文件，显示如何从您定义的其他模块导入（此处未显示模块的代码，请参阅 [此存储库](https://github.com/langchain-ai/langgraph-example-pyproject) 以查看其实陈）：
