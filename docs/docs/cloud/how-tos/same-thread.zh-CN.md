# 如何在同一线程上运行多个代理 (How to run multiple agents on the same thread)

在 LangGraph Platform 中，线程并不显式与特定代理关联。
这意味着您可以在同一线程上运行多个代理，这允许不同的代理从初始代理的进度继续。

在此示例中，我们将创建两个代理，然后在同一线程上调用它们。
您将看到第二个代理将使用第一个代理在线程中生成的 [检查点](https://langchain-ai.github.io/langgraph/concepts/low_level/#checkpointer-state) 中的信息作为上下文进行响应。
...
## 在线程上运行助手 (Run assistants on thread)

### 运行 OpenAI 助手 (Run OpenAI assistant)
...
### 运行默认助手 (Run default assistant)

现在，我们可以在默认助手上运行它，并看到第二个助手知道初始问题，并且可以回答问题“and you?”：
